...
### 11.2 构建卡在解压 CMake 缓存

本仓库现在从 `docker/cuda-builder/deps/` 里的本地 tarball 安装 CMake。若卡住，请检查：

- `docker/cuda-builder/deps/cmake-3.26.0-linux-x86_64.tar.gz` 是否已经存在于构建上下文
- `scripts/prepare-third-party-cache.sh` 是否已刷新 CMake tarball
- `scripts/build-builder-image.sh` 是否仍然指向本地 Docker 构建上下文
