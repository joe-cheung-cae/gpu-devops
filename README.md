...
- `cuda-12`: jobs pinned to the CUDA 12 platform baseline

Single-GPU jobs should use `gpu`, `cuda`, `cuda-12`.

Multi-GPU jobs should use `gpu-multi`, `cuda`, `cuda-12`. The initial implementation treats the multi-GPU pool as a separate Runner registration with stricter concurrency.
...
`<registry>/<namespace>/cuda-builder:cuda12.4-cmake3.30-ubuntu22.04`
