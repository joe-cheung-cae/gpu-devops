...
assert_contains "${ROOT_DIR}/README.md" "run as the current host caller UID/GID by default"
assert_contains "${ROOT_DIR}/README.md" "CUDA_CXX_ALLOW_ROOTFUL_DOCKER=1"
assert_contains "${ROOT_DIR}/README.md" "does not change the Docker daemon itself into rootless mode"
assert_contains "${ROOT_DIR}/docs/usage.en.md" "run as the current Linux caller UID/GID by default"
assert_contains "${ROOT_DIR}/docs/usage.en.md" "CUDA_CXX_ALLOW_ROOTFUL_DOCKER=1"
assert_contains "${ROOT_DIR}/docs/usage.en.md" "It is not the same as running a rootless Docker daemon"
assert_contains "${ROOT_DIR}/docs/usage.zh-CN.md" "默认都会继承当前 Linux 调用者的 UID/GID"
assert_contains "${ROOT_DIR}/docs/usage.zh-CN.md" "CUDA_CXX_ALLOW_ROOTFUL_DOCKER=1"
assert_contains "${ROOT_DIR}/docs/usage.zh-CN.md" "这并不等同于把 Docker daemon 切换成 rootless 模式"
